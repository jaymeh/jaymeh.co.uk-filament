<div class="relative" style="background-image: linear-gradient(to bottom, rgba(255,255,255,1), rgba(255,255,255,0));">
    <div class="py-4 z-50 text-white w-full hidden md:flex md:fixed"
        style="background-image: linear-gradient(to bottom, rgba(0,0,0,1), rgba(255,255,255,0))">
        ...
        <div class="grow menu">
            <div class="flex justify-center items-center box-border font-bold">
                <ul class="box-border text-right" style="width: 200px">
                    <li class="inline-block px-4"><a
                            class="{{ url()->current() == url('/') ? 'font-bold' : '' }} text-white" href="/">Home</a>
                    </li>
                    <li class="inline-block px-4"><a
                            class="{{ url()->current() == url('/blog') ? 'font-bold' : '' }} text-white"
                            href="/blog">Blog</a></li>
                </ul>

                <div class="max-w-xs rounded inline-block px-4">
                    <a href="/">
                        <img src="{{ Vite::asset('resources/images/profile-photo.webp') }}" alt="Profile photo of me."
                            class="rounded-full border-fuchsia-pink border-4" />
                    </a>
                </div>

                <ul class="align-middle box-border" style="width: 200px">
                    <li class="inline-block px-4"><a
                            class="{{ url()->current() == url('/about-me') ? 'font-bold' : '' }} text-white"
                            href="/about-me">About</a></li>
                    <li class="inline-block px-4"><a
                            class="{{ url()->current() == url('/contact') ? 'font-bold' : '' }} text-white"
    ...
    <nav class="md:hidden w-full fixed z-10"
        style="background-image: linear-gradient(to bottom, rgba(0,0,0,1), rgba(255,255,255,0))">
        <div class="align-middle justify-between p-4 mx-auto flex items-center">
            <div>
                <a href="/">
                    <img src="{{ Vite::asset('resources/images/profile-photo.webp') }}" alt="Profile photo of me."
                        class="rounded-full border-fuchsia-pink border-4" style="max-height:60px" />
                </a>
            </div>
            <input id="toggle" type="checkbox" class="-left-16 fixed hidden peer" />
            <div
                class="fixed w-full h-full space-x-8 peer-checked:left-0 duration-300 transition-left md:duration-600 lg:flex top-0 left-full bg-black/90">
                <ul class="pt-4 ">
